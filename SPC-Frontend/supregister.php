...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Initialize cURL
    $ch = curl_init();
    $url = "http://localhost:5268/api/Supplier/AddSupplier";  // Replace with your actual API endpoint
    ...
    if (curl_errno($ch)) {
        ...
        // Check if the response contains a status message or error code
        if (isset($responseData['statusCode'])) {
            // Check if the status message indicates a duplicate email
            if (strpos(strtolower($responseData['statusCode']), 'duplicate email') !== false) {
                echo '<script>alert("Email is already in use. Please use a different email address.");</script>';
            } else {
                echo '<script>alert("' . $responseData['statusCode'] . ': Email is already in use. Please use a different email address.");</script>';
            }
        } else {
...
<head>
    ...
    <title>Supplier Registration - SPC</title>
...
<body>
    <div class="register-container">
        <div class="register-header">
            ...
            <p>Join our network of trusted pharmaceutical suppliers</p>
        </div>
